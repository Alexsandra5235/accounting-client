        ...
        <div id="tab-current" class="tab-content active">
            <div class="card">
                <div class="card-header">
                    ...
                    @if($currentPatientsPaginated->total() > 0)
                        <span class="ml-2 px-3 py-1 bg-blue-100 text-blue-800 text-sm font-medium rounded-full">
                        {{ $currentPatientsPaginated->total() }} записей
                    </span>
                    @endif
                </div>

                <div class="card-body">
                    @if($currentPatientsPaginated->isEmpty())
                    ...
                    @else
                        @include('partials.current-patients-table', ['logs' => $currentPatientsPaginated])

                        {{-- Пагинация для текущих пациентов --}}
                        @if($currentPatientsPaginated->hasPages())
                            @include('partials.pagination', [
                                'paginator' => $currentPatientsPaginated,
                                'tabName' => 'current'
                            ])
                        @endif
                    @endif
        ...
        <div id="tab-discharged" class="tab-content hidden">
            <div class="card">
                <div class="card-header">
                    ...
                    @if($dischargedPatientsPaginated->total() > 0)
                        <span class="ml-2 px-3 py-1 bg-green-100 text-green-800 text-sm font-medium rounded-full">
                        {{ $dischargedPatientsPaginated->total() }} записей
                    </span>
                    @endif
                </div>

                <div class="card-body">
                    @if($dischargedPatientsPaginated->isEmpty())
                        <div class="text-center py-8">
                            <div class="w-20 h-20 mx-auto mb-4 rounded-full bg-gray-100 flex items-center justify-center">
                    ...
                    @else
                        @include('partials.discharged-patients-table', ['logs' => $dischargedPatientsPaginated])

                        {{-- Пагинация для выписанных пациентов --}}
                        @if($dischargedPatientsPaginated->hasPages())
                            @include('partials.pagination', [
                                'paginator' => $dischargedPatientsPaginated,
                                'tabName' => 'discharged'
                            ])
                        @endif
                    @endif
